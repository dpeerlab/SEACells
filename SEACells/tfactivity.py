...
def gene_tf_associations(gene_peak, pwm, 
...
def get_de_genes(adata, groups, thresh, group_key, fc_min=1.5, pval_cut=1e-2):
    ...
    for g in tqdm(groups, total=len(groups)):
        ...
        _compute_de_genes(adata,g, group_dict, ref,
                    group_key, fc_min, pval_cut)
...
def get_gene_set(mc_ad, peak_counts,de_genes, group_key, sub_group,
                 expr_thresh=25, min_peaks=5):
    mc_expr = mc_ad.to_df()
    valid_genes = peak_counts[peak_counts >= min_peaks].index
    
    highly_expressed = mc_expr.columns[(mc_expr.groupby(mc_ad.obs[group_key]).mean().loc[sub_group] > expr_thresh).any()]
...
def fit_lasso_model(gtf, zs, tf_set, gene_set, cells, 
                    test_size=0.20, cv_fold=5, max_iter=10000):
...
def _train_model(cell, X, y, gene_set, res, test_size=0.20, cv_fold=5, max_iter=10000):
    ...
    lasso_cv_model = LassoCV(cv=cv_fold, max_iter=max_iter, n_jobs=16).fit(X_train,y_train)
...
def compute_tf_activity(ad_sub, res, valid_tfs, cells):
    ...
    tf_activities = pd.DataFrame(0.0, index=valid_tfs, columns=cells) # set up container
