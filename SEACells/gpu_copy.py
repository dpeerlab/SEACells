...
class SEACellsGPU:
    ...
    def initialize_archetypes(self):
        ...
        del waypoint_ix, greedy_ix, all_ix, unique_ix, ind
        cp._default_memory_pool.free_all_blocks()
        cp._default_pinned_memory_pool.free_all_blocks()
    ...
    def initialize(self, initial_archetypes=None, initial_assignments=None):
        ...
        if initial_assignments is not None:
            ...
            rows = cp.random.randint(0, k, size=(n, archetypes_per_cell)).reshape(-1)
            columns = cp.repeat(cp.arange(n), archetypes_per_cell)

            A0 = cupyx.scipy.sparse.csr_matrix(
                (cp.random.random(len(rows)), (rows, columns)), shape=(k, n)
            )
            # Normalize axis 0 with l1 norm
            l1_norms  = norm(A0, ord=1, axis=0)
            l1_norms[l1_norms == 0] = 1.0
    ...
    def _updateA(self, Bg, Ag):
        ...
        del t1g, t2g, Kg, Gg, Bg, eg, aminsG
        cp._default_memory_pool.free_all_blocks()
        cp._default_pinned_memory_pool.free_all_blocks()
